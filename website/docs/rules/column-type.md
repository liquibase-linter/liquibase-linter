...
- `column-condition` - (string) - [Spring EL expression](https://www.baeldung.com/spring-expression-language) that should resolve to a boolean, which if provided will decide whether the rule should be applied or not to the evaluated column. The expression scope is [`ColumnConfig`](https://github.com/liquibase/liquibase/blob/master/liquibase-standard/src/main/java/liquibase/change/ColumnConfig.java) object
...
```json
{
  "rules": {
    "column-type": {
      "pattern": "^int$",
      "column-condition": "name == 'id'",
      "error-message": "Column type '%s' of column with name '%s' should be 'int'"
    }
  }
}
```
