...
class ColumnNameRuleTest {
    ...
    @Test
    void shouldHaveName() {
        assertThat(rule.getName()).isEqualTo("column-name");
    }
    ...
    void columnNameNameMustFollowPattern(String changeName, Function<String, Change> changeFromColumnName) {
        rule.configure(RuleConfig.builder().withPattern("^(?!COL)[A-Z_]+(?<!_)$").build());

        assertThat(rule.invalid(changeFromColumnName.apply("COL_INVALID"))).isTrue();
        assertThat(rule.getMessage(changeFromColumnName.apply("COL_INVALID"))).isEqualTo("Column name 'COL_INVALID' does not follow pattern '^(?!COL)[A-Z_]+(?<!_)$'");

        assertThat(rule.invalid(changeFromColumnName.apply("VALID"))).isFalse();
    }

    @DisplayName("Column name rule should support formatted error message with pattern arg")
    @ParameterizedTest(name = "With {0}")
    @ArgumentsSource(ChangeFromColumnNameArgumentsProvider.class)
    void columnNameNameRuleShouldReturnFormattedErrorMessage(String changeName, Function<String, Change> changeFromColumnName) {
        Change change = changeFromColumnName.apply("COL_INVALID");
        rule.configure(RuleConfig.builder().withPattern("^(?!COL)[A-Z_]+(?<!_)$").withErrorMessage("Column name '%s' must follow pattern '%s'").build());

        assertThat(rule.getMessage(change)).isEqualTo("Column name 'COL_INVALID' must follow pattern '^(?!COL)[A-Z_]+(?<!_)$'");
    }
